        ...
        <div className="fixed z-10 inset-0 overflow-y-auto flex items-center justify-center">
            ...
            <AnimatedModal style={springProps} className="rounded-lg px-4 pt-5 pb-4 text-center overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full sm:p-6 bg-app-color text-inside font-marvel">
                ...
                <form onSubmit={handleSubmit} className="mt-4">
                    ...
                    <button type="submit" className="inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-button-pink text-lg font-medium text-inside hover:bg-button-yellow focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-button-yellow sm:text-sm">Login</button>
                </form>
                {/* Close the modal when the link is clicked */}
                <p className="mt-4 text-sm">Don't have an account? <Link to="/signup" className="text-app-color hover:underline" onClick={onClose}>Sign up for YourApp</Link></p>
                <button type="button" className="mt-5 inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-app-color text-lg font-medium text-inside hover:bg-button-yellow focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-button-yellow sm:text-sm" onClick={onClose}>Close</button>
            </AnimatedModal>
        </div>
    );
};
